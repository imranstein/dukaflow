...
That closes v1.1: all three ADR-002 §10 candidates now have an answer — two built and verified live, one considered and rejected with the reasoning on record. `Docs/PLAN.md` and `ROADMAP.md` both marked the section done. Nothing queued next; `v1.0.0` is still waiting on the live demo, same as before this whole detour started.
