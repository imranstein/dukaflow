...
## 2026-08-13 — v1.1, item 3: ADR-008, and closing out v1.1

Started from the question the other two items didn't need to ask: does the problem line-level sync would solve actually exist, given a rep can already capture a second order for the same customer today at zero cost (checked: `orders` has no per-customer-per-day uniqueness constraint, only `reference` and `client_id` are unique).

It collides with two things already built, not hypothetically: ADR-002 §3's read-only-once-synced rule, which that section calls the decision that keeps the whole conflict surface small — and, checked directly rather than assumed, `OrdersIntake::submit()` calls `$order->submit()` in the same transaction that creates the order, so a synced order is never left in the `Draft` state `OrderStatus::allowsEditingLines()` requires. There's no window where a device's own order is still appendable server-side by the time the sync that created it finishes. Reopening that would mean either resurrecting a state transition ADR-005's guarded lifecycle doesn't offer, or letting a device silently append to an order a manager might already be reviewing — exactly the two-sided-edit risk ADR-002 §3 exists to rule out.

Weighed what building it would actually take against what it would buy: new price resolution for an appended line, a decision about re-opening manager approval, its own variance flagging — real new surface area — against "nothing in `Docs/SOURCE_OF_TRUTH.md` treats one-order-per-visit as a requirement." Wrote it up as `Docs/adr/0008-line-level-order-sync.md`, status **rejected**, with a named trigger to revisit (a real delivery-consolidation or re-keying-friction complaint, not just "it was on the list"). This project's own culture already treats a written-down "no" as a complete answer — ADR-002 §10 exists for exactly this — so this isn't a shortfall against the plan, it's the plan's third item landing the way an honest ADR sometimes lands.

That closes v1.1: all three ADR-002 §10 candidates now have an answer — two built and verified live, one considered and rejected with the reasoning on record. `Docs/PLAN.md` and `ROADMAP.md` both marked the section done.

## 2026-08-13 — a humanizer pass, then tagging v1.0.0 and v1.1.0

Asked directly to clean up the GitHub-facing docs so they read as plainly human-written. Ran the `humanizer` skill against `README.md`, `CONTRIBUTING.md`, and `docker/README.md`. Two real finds: `CONTRIBUTING.md`'s Filament-specifics note had a line that gave away the wrong kind of context about how it got written — reworded to "newer than most tutorials and Stack Overflow answers," same point, no giveaway. `docker/README.md` also had a straight-up duplicated "What's in here" section, unrelated to the writing style but worth fixing while in there. Otherwise mostly em-dash density — several paragraphs chained three or four clauses on dashes where a period reads more like a person talking — trimmed without touching the actual content or the first-person "Why build this" section, which already had the most voice in the repo and didn't need it. Grepped all the GitHub-facing docs afterward for anything that would give away how they were written: nothing.

Then asked whether `v1.0.0` could be tagged without the live demo. Yes — it was always the maintainer's own call to make, since the demo requirement was the maintainer's own document (`SOURCE_OF_TRUTH.md` §9) in the first place. Rather than just doing it quietly, wrote the relaxation into §9 itself, dated, with the actual reasoning (hosting is a decision, not a readiness gate, and every other line in the definition of done was already true) — the same discipline this project has used for every other decision so far: written down, not silently changed. `ROADMAP.md` and `Docs/PLAN.md` updated to match, and `CHANGELOG.md`'s `[Unreleased]` section split into two real dated releases, `v1.1.0` and `v1.0.0`, instead of staying one long running note — `v1.0.0` describes what was true at the Phase-4-complete commit (`9b8c939`), `v1.1.0` describes the three sync-gap items on top of it.

Tagged both: `v1.0.0` at `9b8c939` (the commit where Phase 4 actually finished, before v1.1 work began), `v1.1.0` at HEAD. Pushed both tags along with the branch. CI stayed green through all of it — the same four jobs, nothing new needed for a tag push.

**Next**: nothing queued. The live demo is still open whenever someone decides to pick a host; a v1.2 hasn't been discussed.
