...
A conflict is the one case idempotency alone can't resolve: the same client-generated id, but different content. That can only mean the id got reused for something else, and there's no principled way to guess which version is right — so nothing is guessed. The mismatch is written to `sync_conflicts` and the rep's own device shows it — a count badge in the PWA's sync status bar — rather than being silently swallowed. There's no back-office conflicts queue for a manager yet; today the record exists for whoever built or is debugging the sync layer to query directly, and the rep-facing badge is what tells anyone a conflict happened at all. This is also why the design keeps a hard rule that makes conflicts rare in the first place: **once an order has synced, it's read-only from the device's side.** Any correction to an order after that happens in the back office, through the same guarded actions a manager would use anyway — that's a separate thing from resolving a conflict, since a conflicted submission was never written as an order in the first place.
