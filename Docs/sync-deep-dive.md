...
A conflict is the one case idempotency alone can't resolve: the same client-generated id, but different content. That can only mean the id got reused for something else, and there's no principled way to guess which version is right — so nothing is guessed. The mismatch is written to `sync_conflicts` — the rejected payload alongside its hash, not just the hash — and it surfaces two places: a count badge in the rep's own PWA sync status bar, and a back-office queue under Sync → Conflicts, where a manager sees the rejected payload next to the row that actually won (the same `(client_id, entity_type)` lookup the push handler uses for idempotency) and can mark it reviewed. This is also why the design keeps a hard rule that makes conflicts rare in the first place: **once an order has synced, it's read-only from the device's side.** Any correction to an order after that happens in the back office, through the same guarded actions a manager would use anyway — that's a separate thing from resolving a conflict, since a conflicted submission was never written as an order in the first place.
...
- **No dependence on the Background Sync API.** It's Chromium-only, so nothing here registers for it — the queue drains on the app-open pull, the `online` event, a periodic timer, and the manual button instead, and any one of those alone is enough. Nothing is lost by skipping it: those four triggers cover the same moments Background Sync would, on every browser instead of some.
